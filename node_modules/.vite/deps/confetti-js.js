...
function ConfettiGenerator(params) {
  ...
  if (params) {
    ...
    if (params.animate !== void 0 && params.animate !== null)
      appstate.animate = params.animate;
    if (params.respawn !== void 0 && params.respawn !== null)
  ...
  function rand(limit, floor) {
    if (!limit)
      limit = 1;
    var rand2 = Math.random() * limit;
    return !floor ? rand2 : Math.floor(rand2);
  }
  var totalWeight = appstate.props.reduce(function(weight, prop) {
    return weight + (prop.weight || 1);
  }, 0);
  function selectProp() {
    var rand2 = Math.random() * totalWeight;
  ...
  var _render = function() {
    ...
    function draw() {
      ...
      for (var i2 in particles)
        particleDraw(particles[i2]);
      update();
      if (appstate.animate)
        requestAnimationFrame(draw);
    }
    function update() {
      for (var i2 = 0; i2 < appstate.max; i2++) {
        var p = particles[i2];
        if (p) {
          if (appstate.animate)
            p.y += p.speed;
          ...
          if (p.speed >= 0 && p.y > appstate.height || p.speed < 0 && p.y < 0) {
            if (appstate.respawn) {
              particles[i2] = p;
              particles[i2].x = rand(appstate.width, true);
              particles[i2].y = p.speed >= 0 ? -10 : parseFloat(appstate.height);
            } else {
              particles[i2] = void 0;
            }
          }
      ...
      if (particles.every(function(p2) {
        return p2 === void 0;
      })) {
        _clear();
      }
...
var index_es_default = ConfettiGenerator;
export {
  index_es_default as default
};
//# sourceMappingURL=confetti-js.js.map
